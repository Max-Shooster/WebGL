...
function createShapes() {
  ...
  myVespa = new vespa();
  myVespa.VAO = bindVAO (myVespa, programs[0]);
...
function setUpTextures(){
    ...
    vespaTexture = gl.createTexture();

    // load the actual image
    var vespaImage = document.getElementById ('vespa-texture');

    // bind the texture so we can perform operations on it
    gl.bindTexture (gl.TEXTURE_2D, vespaTexture);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, vespaImage.width, vespaImage.height,
       0, gl.RGBA, gl.UNSIGNED_BYTE, vespaImage);
...
function drawShapes() {
    ...
    // drawing the Vespa rotating around Y  180 degrees
    let vespaMatrix= glMatrix.mat4.create();
    // send the Vespa model matrix to the shader and draw.
    glMatrix.mat4.translate(vespaMatrix, vespaMatrix, [20, -30, 0]);
    gl.uniformMatrix4fv (programs[0].uModelT, false, vespaMatrix);
    gl.bindVertexArray(myVespa.VAO);
    gl.activeTexture (gl.TEXTURE0);
    gl.bindTexture (gl.TEXTURE_2D, vespaTexture);
    gl.uniform1i (programs[0].uVespaTexture, 0);
    gl.drawElements(gl.TRIANGLES, myVespa.indices.length, gl.UNSIGNED_SHORT, 0);
}
  ...
  function initPrograms() {
    ...
    programs[0].uvespaTexture = gl.getUniformLocation (programs[0], 'vespaTexture');
...
function getShader(id) {
  ...
  function initProgram(vertex_id, fragment_id) {
    ...
    program.uvespaTexture = gl.getUniformLocation (program, 'vespaTexture');
